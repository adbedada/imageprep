...
def list_path_to_files(path_to_folders, output_file_name, save=False):
    """
     Save the path to files (images or labels) in one text file.

    Args:
        path_to_folders: path to the folder containing images or labels
        output_file_name: name of output text file
        save: option to save list to a text file

    Returns: A text file with a list of path to files
...
def read_image(file, as_array=True):
    """
     Read image and returns a numpy array.
...
def read_label_as_dict(file, ext=".txt"):
    """
     Read a label file in text format as a dictionary.
...
def read_label_as_list(file, ext=".txt"):
    """
     Read a label file in text format as a list.
    :param file: Name of the label file
    :param ext: Name of the file extension. Defaulted to text
